...
void proximity(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol, std::vector<bool> &processed, int index, pcl::PointIndices &cluster)
{
	if (processed[index])
	{
		return;
	...
	for (int i = 1; i < nearby_points.size(); i++)
	{
		...
		{
			proximity(points, tree, distanceTol, processed, nearby_points[i], cluster);
		}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	for (int i = 0; i < points.size(); i++)
	{
		...
		proximity(points, tree, distanceTol, processed, i, cluster);
